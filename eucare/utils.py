...
def random_directed_set(edges: HalfEdgeGraph | Iterable[HalfEdge]) -> set[HalfEdge]:
    ...
    """
    halfedges: Iterable[HalfEdge] = edges.halfedges if isinstance(edges, HalfEdgeGraph) else edges
...
def print_attribute_info(objs: HalfEdgeGraph | Iterable[AttributeObject]) -> None:
    ...
    attribute_dict: dict[str, set[object]] = defaultdict(set)
    for obj in obj_list:
        ...
        for key, val in obj.attributes.items():
            try:
                ...
                pass
            counter[key] += 1
    logger.info("%d Objects", len(obj_list))
