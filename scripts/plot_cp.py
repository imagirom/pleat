...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--score", action="store_true", help="If true, scores the crease pattern and its border.")
    parser.add_argument("-d", "--draw_creases", action="store_true", help="If true, draws the (interior) creases.")
    parser.add_argument("-b", "--draw_borders", action="store_true", help="If true, draws the borders.")
    parser.add_argument(
        "-c", "--convert", action="store_true", help="If true, automatically converts .svg to .hpgl if needed"
    )
    parser.add_argument(
        "-p", "--plotter", type=str, default=None, help="Path to plotter. By default, will look at /dev/usb/lp*."
    )
    parser.add_argument(
        "-r",
        "--repetitions",
    ...
    for i, directory in enumerate(args.dirs):
        ...
        files = [
            os.path.join(directory, f) for f in ["cp_for_cutting", "cp_for_cutting_interior", "cp_for_cutting_borders"]
        ]
        ...
        for _ in range(args.repetitions):
            ...
            if args.score:
                ...
                if os.path.exists(hpgl_files[0]):
                    ...
                    assert os.path.exists(hpgl_files[1]) and os.path.exists(hpgl_files[2]), (
                        f"Some files for scoring are missing: " f"{[(f, os.path.exists(f)) for f in hpgl_files]}"
                    )
            ...
            if args.draw_creases and args.draw_borders:
                ...
                if os.path.exists(hpgl_files[0]):
                    ...
                    assert os.path.exists(hpgl_files[1]) and os.path.exists(hpgl_files[2]), (
                        f"Some files for drawing are missing: " f"{[(f, os.path.exists(f)) for f in hpgl_files]}"
                    )
